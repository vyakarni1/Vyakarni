...
serve(async (req) => {
  ...
  try {
    const { textContent, wordReplacements } = await req.json();
    ...
    let preprocessedText = textContent;
    ...
    console.log('Processing PDF text for grammar correction');
    console.log('Original text length:', textContent.length);
    console.log('After word replacements length:', preprocessedText.length);
...
- PDF की मूल फॉर्मेटिंग संरचना को बनाए रखें`
          },
          {
            role: 'user',
            content: `इस हिंदी PDF टेक्स्ट को सुधारें:\n\n${preprocessedText}`
          }
        ],
        max_tokens: 4000,
        temperature: 0.1,
    ...
    console.log('PDF grammar correction completed');
    ...
    console.error('Error in pdf-grammar-check function:', error);
    return new Response(
      JSON.stringify({ error: `PDF grammar correction failed: ${error.message}` }), 
      { 
        status: 500, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      }
