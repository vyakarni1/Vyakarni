-- Step 1: Enable pg_net extension for HTTP functionality
CREATE EXTENSION IF NOT EXISTS pg_net SCHEMA net;

-- Step 2: Drop the problematic trigger that blocks user registration
DROP TRIGGER IF EXISTS trigger_welcome_email_on_profile_insert ON public.profiles;

-- Step 3: Drop the problematic function that causes signup failures
...
-- Step 5: Create a non-blocking trigger for welcome emails
