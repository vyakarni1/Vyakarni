-- Step 1: Drop the problematic trigger that blocks user registration
DROP TRIGGER IF EXISTS trigger_welcome_email_on_profile_insert ON public.profiles;

-- Step 2: Drop the problematic function that causes signup failures  
DROP FUNCTION IF EXISTS public.trigger_welcome_email();

-- Step 3: Enable pg_net extension (let it create its own schema)
CREATE EXTENSION IF NOT EXISTS pg_net;

-- Step 4: Update the auth webhook function to work with auth state changes
...
DECLARE
  user_profile RECORD;
BEGIN
  -- Only proceed if this is an email confirmation (email_confirmed_at changed from null to not null)
  IF OLD.email_confirmed_at IS NULL AND NEW.email_confirmed_at IS NOT NULL THEN
    -- Get user profile information
    SELECT id, email, name INTO user_profile
    FROM public.profiles 
    WHERE id = NEW.id;
    
    IF FOUND THEN
      -- Send welcome email safely (won't block if it fails)
      PERFORM public.send_welcome_email_safe(
        user_profile.id, 
        user_profile.email, 
        user_profile.name
      );
...
-- Step 6: Create a function for manual bulk welcome email sending
