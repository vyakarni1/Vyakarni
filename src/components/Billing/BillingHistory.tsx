...
const BillingHistory = () => {
  ...
  const fetchBillingData = async () => {
    try {
      ...
      } = await supabase.from('word_purchases').select('*').eq('user_id', user?.id).order('purchase_date', {
        ascending: false
      });
      ...
      const {
        data: usageData,
        error: usageError
      } = await supabase.from('word_usage_history').select('*').eq('user_id', user?.id).order('created_at', {
        ascending: false
      }).limit(50);
  ...
  const exportData = () => {
    ...
    const headers = activeTab === 'purchases' ? ['Date', 'Words', 'Amount', 'Status', 'Payment ID'] : ['Date', 'Words Used', 'Action Type', 'Text Preview'];
    const csvContent = [headers.join(','), ...dataToExport.map(item => {
      if (activeTab === 'purchases') {
        const purchase = item as WordPurchase;
        return [format(new Date(purchase.purchase_date), 'dd/MM/yyyy'), purchase.words_purchased, `₹${purchase.total_amount}`, purchase.status, purchase.razorpay_payment_id || 'N/A'].join(',');
      } else {
        const usage = item as UsageHistory;
        return [format(new Date(usage.created_at), 'dd/MM/yyyy HH:mm'), usage.words_used, getActionTypeLabel(usage.action_type), (usage.text_processed || '').substring(0, 50).replace(/,/g, ';')].join(',');
      }
    })].join('\n');
    ...
    return <Card className="animate-pulse">
        ...
        <CardContent>
          <div className="space-y-4">
            {Array.from({
            length: 5
          }).map((_, i) => <div key={i} className="h-12 bg-gray-200 rounded"></div>)}
          </div>
        </CardContent>
      </Card>;
  }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              <Button variant={activeTab === 'purchases' ? 'default' : 'outline'} size="sm" onClick={() => setActiveTab('purchases')}>
                खरीदारी
              </Button>
              <Button variant={activeTab === 'usage' ? 'default' : 'outline'} size="sm" onClick={() => setActiveTab('usage')}>
                उपयोग
              </Button>
        ...
        <CardContent>
          {activeTab === 'purchases' ? <div className="space-y-4">
              {purchases.length === 0 ? <div className="text-center py-8 text-gray-500">
                  <Calendar className="h-12 w-12 mx-auto mb-4 opacity-50" />
                  <p>अभी तक कोई खरीदारी नहीं की गयी है</p>
                </div> : <Table>
                  <TableHeader>
                    <TableRow>
              ...
              {usageHistory.length === 0 ? <div className="text-center py-8 text-gray-500">
                  <Calendar className="h-12 w-12 mx-auto mb-4 opacity-50" />
                  ...
                  <TableBody>
                    {usageHistory.map(usage => <TableRow key={usage.id}>
                        <TableCell>
