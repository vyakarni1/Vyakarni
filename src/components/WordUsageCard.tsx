...
const WordUsageCard = () => {
  ...
  if (loading) {
    return <Card className="border-0 shadow-lg bg-gradient-to-br from-white to-gray-50">
        <CardHeader>
  ...
  const usagePercentage = balance.total_words_available > 0 ? Math.min(100, estimatedWordsUsedThisMonth / (balance.total_words_available + estimatedWordsUsedThisMonth) * 100) : 0;
  const isHighUsage = usagePercentage > 70;
  return <Card className="border-0 shadow-lg bg-gradient-to-br from-white to-blue-50 relative overflow-hidden">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-700">संभावित मासिक उपयोग</span>
            <span className="text-sm font-semibold text-blue-600">
              ~{estimatedWordsUsedThisMonth.toLocaleString()} शब्द
            </span>
          </div>
          
          <Progress value={usagePercentage} className={`h-3 ${isHighUsage ? 'bg-orange-100' : 'bg-blue-100'}`} />
        ...
        {isHighUsage && <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
        ...
        {!isHighUsage && balance.total_words_available > 500 && <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
